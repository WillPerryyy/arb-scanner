...
async def run_full_scan() -> tuple[list[ArbitrageOpportunity], list[EvEdgeOpportunity], list[ValueOpportunity], list[ScannerStatus], dict[str, int]]:
    ...
    #
    # pin_c covers outcome B, kal_c covers outcome A (complementary hedge pair).
    # oracle_key = (pin_c.parent_event_id, not pin_c.is_yes_side) → Pinnacle prob for A.
    ...
    for contract_a, contract_b, _score, _ in hedge_pairs:
        ...
        if oracle_price is not None and kal_map_key not in kalshi_oracle_map:
    ...
    for contract_a, contract_b, _score, _ in hedge_pairs:
        ...
        if oracle_price is not None and poly_key not in polymarket_oracle_map:
    ...
    # Soccer 3-way markets: Kalshi emits `not_X` contracts (e.g. "not_SJ") which
    # appear in hedge pairs paired with Pinnacle's `X` line.  The complementary
    # `X YES` contracts (e.g. "SJ wins directly") are never in hedge_pairs and
    # therefore never receive an oracle entry — meaning they're invisible to the
    # value pass below.
    #
    # Fix: for any (market_id, is_yes_side) that has an oracle but whose mirror
    # (market_id, not is_yes_side) does not, derive the mirror from P(X) = 1 − P(¬X).
...
async def scan_sharp_value(
    ...
    # Build kalshi_oracle_map — identical logic to run_full_scan
    kalshi_oracle_map: dict[tuple[str, bool], float] = {}
    for contract_a, contract_b, _score, _ in hedge_pairs:
        is_pin_a = contract_a.platform == Platform.PINNACLE
        is_pin_b = contract_b.platform == Platform.PINNACLE
